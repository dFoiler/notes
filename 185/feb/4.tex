...
\begin{lemma}
	...
	\begin{listalph}
		\item Then $f$ is continuous at $w$ if and only if eevery sequence $\{z_n\}\subseteq X$ such that $z_n\to z$ implies $f(z_n)\to f(z)$.
		\item We have that $f$ is continuous if and only if every open open set $U\subseteq\CC$ has $f^{-1}(U)$ open in $X$.
		\item We have that $f$ is continuous on $X$ if and only if each closed set $V\subseteq X$ has $f^{-1}(V)$ closed in $X$.
		\item We have that $f$ is continuous on $X$ if and only if, for each $\varepsilon>0$, we have that $f^{-1}(B(z,\varepsilon))$ is open in $X$.
	\end{listalph}
\end{lemma}
\begin{proof}
	...
	\begin{listalph}
		...
		It follows from $z_n\to w$ that there exists some $N$ such that $n>N$ has
		\[|z_n-w|<\delta\implies|f(z_n)-f(z)|<\varepsilon,\]
	...
	\end{listalph}
	We refer the other parts of the proof to Eterovi\'c.
\end{proof}
