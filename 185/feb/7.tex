...
\begin{proof}
	...
	\begin{itemize}
		...
		However, we claim there is no continuous path $\gamma:[0,1]\to X$ with $\gamma(0)=x_1$ and $\gamma(1)=x_2$. Indeed, the image of $\gamma([0,1])$ must be connected, but then we can disconnect $\gamma([0,1])$ by $U_1$ and $U_2$: $\gamma([0,1])\subseteq U_1\cup U_2$ and $U_\bullet\cap\gamma([0,1])=\{x_\bullet\}$ and $U_1\cap U_2=\emp$.
