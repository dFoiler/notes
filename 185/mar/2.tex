...
\begin{definition}[\texorpdfstring{$\exp$}{exp}]
...
\begin{proof}
	...
	Lastly, to show that $\exp$ is analytic, we need to show that $\exp$ can be locally expanded as a power series. In fact, for any $z_0\in\CC$, we claim that $\exp$ can be globally expanded as a power series. In particular, we need to be able to write
	\[\exp(z)=\sum_{k=0}^\infty a_k(z-z_0)^k,\]
	where the series on the right converges for any $z\in\CC$. For this, we expand
	\[\exp(z+z_0)=\sum_{n=0}^\infty\frac1{k!}(z+z_0)^n.\]
	...
	\[\sum_{n=0}^\infty\frac1{k!}(z+z_0)^n=\sum_{n=0}^\infty\left(\sum_{k+\ell=n}\frac1{k!\ell!}z^kz_0^\ell\right)\stackrel*=\sum_{k=0}^\infty\left(\sum_{\ell=0}^\infty\frac1{k!\ell!}z_0^\ell\right)z^k,\]
	...
	\[a_{n,k}:=\begin{cases}
		\frac{1}{k!(n-k)!}z^kz_0^{n-k} & k\le n, \\
		0 & k>n
	\end{cases}\]
	so that we are interested in exchanging the order of the summation
	\[\sum_{n=0}^\infty\left(\sum_{k+\ell=n}\frac{1}{k!\ell!}z^kz_0^\ell\right)=\sum_{n=0}^\infty\sum_{k=0}^\infty a_{n,k}.\]
	Well, for fixed $n$, we see that $\sum_{k=0}^\infty|a_{n,k}|$ is a finite sum and hence converges. And further, we see that
	\[\sum_{n=0}^\infty\sum_{k=0}^\infty|a_{n,k}|=\sum_{n=0}^\infty\left(\sum_{k=0}^n\frac{\binom nk}{n!}|z|^k|z_0|^{n-k}\right)=\sum_{n=0}^\infty\frac1{n!}(|z|+|z_0|)^n=\exp(|z|+|z_0|),\]
	which converges because $\exp$ has infinite radius of convergence. As such, \autoref{lem:doublesums} tells us that
	\[\exp(z+z_0)=\sum_{n=0}^\infty\sum_{k=0}^\infty a_{k,\ell}=\sum_{k=0}^\infty\sum_{n=0}^\infty a_{k,\ell}=\sum_{k=0}^\infty\sum_{n=k}^\infty\frac1{k!(n-k)!}z^kz_0^{n-k}.\]
	The inner sums we may simplify as $\sum_{n=k}^\infty\frac1{k!(n-k)!}z^kz_0^{n-k}=z^k\sum_{\ell=0}^\infty\frac1{k!\ell!}z_0^\ell$, so we do indeed find that
	\[\exp(z+z_0)=\sum_{k=0}^\infty\left(\sum_{\ell=0}^\infty\frac1{k!\ell!}z_0^\ell\right)z^k,\]
	for any $z\in\CC$. In particular, plugging in $z-z_0$ tells us that
	\[\exp(z)=\sum_{k=0}^\infty\left(\sum_{\ell=0}^\infty\frac1{k!\ell!}z_0^\ell\right)(z-z_0)^k,\]
...
\begin{definition}[{\texorpdfstring{$\sin$}{sin} and \texorpdfstring{$\cos$}{cos}}]
...
\begin{definition}[\texorpdfstring{$\pi$}{pi}]
...
\begin{definition}[\texorpdfstring{$\arctan$}{arctan}]
...
\begin{proposition}
	For any $z\in\CC^\times$, there exists unique real numbers $r>0$ and $\theta\in[-\pi,\pi)$ such that $z=r\exp(i\theta)$.
\end{proposition}
\begin{proof}
