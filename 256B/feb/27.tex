...
\begin{theorem} \label{thm:qcoh-cohom}
	Fix an affine scheme $X=\Spec A$. If $\mc F$ is a quasicoherent sheaf, then $H^i(X,\mc F)=0$ for $i>0$.
\end{theorem}
The main input to this result is the following result.
\begin{proposition} \label{prop:inj-qcoh-sheaf-is-flasque}
	Fix a ring $I$ and an injective $A$-module $I$. Then the quasicoherent sheaf $\widetilde I$ on $\Spec A$ is flasque.
\end{proposition}
Let's see how \Cref{prop:inj-qcoh-sheaf-is-flasque} implies \Cref{thm:qcoh-cohom}.
...
\begin{lemma}
	...
	\[\Gamma_Z(\widetilde M)\coloneqq\left\{m\in M:a^nm=0\text{ for some }n>0\text{ for all }a\in\mf a\right\}.\]
\end{lemma}
\begin{proof}
...
\end{lemma}
\begin{proof}
	Fix an injective $I$, and set $J\coloneqq\Gamma_\mf a(F)$. By the homework, it is enough to show that any ideal $\mf b\subseteq A$ with a morphism $\varphi\colon\mf b\to J$ can be extended to a morphism $A\to J$.
