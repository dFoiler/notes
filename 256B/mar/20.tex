...
\begin{theorem}[Serre duality for \texorpdfstring{$\PP^n_k$}{Pn}]
...
\begin{proof}
	...
	\begin{itemize}
		...
		For $0<i<n$, this vanishes automatically by \Cref{thm:proj-cohom}; for $i=n$, we compute these factors are $H^0(X,\OO(-q_k))=0$ still from \Cref{thm:proj-cohom}. Thus, the surjection $\mc E\onto\mc F$ witnesses that $\op{Ext}^i(-,\omega)$ is co-effaceable.
