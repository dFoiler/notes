...
\begin{proof}
	...
	\begin{enumerate}
		...
		\end{tikzcd}\]
		The rightmost terms vanish because $\mc E$ is locally free, and the top row is exact for free. Further, the two left maps are isomorphisms by the inductive hypothesis, so we will be done if we can show that the bottom row is exact in high enough twisting. Well, large twisting kills cohomology, so for $n$ large enough, taking global sections will be exact, which is what we wanted.
		\qedhere
	\end{enumerate}
\end{proof}
\begin{remark}
	...
	However, by twisting $\mc G$ high enough, we kill these cohomology objects for $n$ large enough, which provides the needed equality.
\end{remark}
