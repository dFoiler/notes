...
\begin{proof}
	...
	\end{align*}
	For all $v\notin\{\ell_i,\ldots,\ell_{i+r}\}$, the local Tate pairing will vanish because both classes are unramified. Further, for $v\in\{\ell_{i+1},\ell_{i+r-1}\}$, both classes are transverse, so the pairing still vanishes. Thus, we are left with
	\[\langle c,c(\ell_i\cdots\ell_{i+r})\rangle_{\ell_i}=-\langle c,c(\ell_i\cdots\ell_{i+r})\rangle_{\ell_{i+r}}.\]
	The left-hand side does not vanish because they come from the same eigenspaces, so the right-hand side does not vanish, and the claim follows by \Cref{thm:kolyvagin-system-prop}.\todo{}
\end{proof}
\begin{remark}
