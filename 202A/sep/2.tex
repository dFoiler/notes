...
\begin{proof}
	...
	\begin{itemize}
		...
		\[\Bigg(\int_0^1|f(t)|\,dt\Bigg)^p\le\int_0^1|f(t)|^p=\norm f_p,\]
...
\begin{proof}
	...
	Notably, as $c\to\infty$, we have $\norm f_p\le2^{1/p}\cdot c^{1/p-1}$ goes to $0$, but $|\varphi(f)|=c$ grows unbounded. Thus, $\varphi$ is discontinuous.
\end{proof}
\begin{remark}
