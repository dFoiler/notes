...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Suppose $p\ge1$ is finite. We use H\"older's inequality without proof, which tells us that
		\[|\varphi_g(f)|=\norm{fg}_1\le\norm f_p\cdot\norm g_{1/(1-1/p)},\]
		where $\norm g_{1/(1-1/p)}$ is finite because $g$ is continuous on a compact set and hence bounded.
		\qedhere
	\end{itemize}
\end{proof}
\begin{remark}
	...
	is linear and continuous for $\norm{\cdot}_\infty$ but not for $\norm{\cdot}_p$ where $p$ is finite. To see the continuity at $p=\infty$, use \autoref{lem:boundediffcont}: note $\varphi(f)\le\norm f_\infty$. However, the lack of continuity for $p<\infty$ can be seen by choosing $f$ to have a sharp bump at $t=t_0$, which allows $\varphi(f)$ to be large even if $\norm f_p$ is small.
\end{remark}
\begin{remark}
