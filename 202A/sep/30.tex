...
\begin{proof}
	...
	% Note that having $y\in U_i$ implies that $d(f(y),f(x_i))\le\varepsilon/4$ by construction of $U_i$, and for any given $f(x_i)$, there is a function $g_i\in S_i$ such that $d(f(x_i),g_i(x_i))<\varepsilon/4$ by construction of the $S_i\subseteq S$. Defining $\psi$ by $\psi_i\coloneqq g_i$, we see that any $y\in U_i$ for any $i$ has
	% \[d(f(y),f(x_i))\le d(f(y),g_i(x_i))+d(g_i(x_i),f(x_i))\le\varepsilon/2.\]
