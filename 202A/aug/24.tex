...
\begin{itemize}
	...
	\item There is no particular text for this course, and any given text covers more than we have time for. That said, we will (very) loosely follow Lang's \textit{Real and Functional Analysis}, but it is helpful to have a number of different expositions around.
...
\begin{definition}[Extended metric]
	An \textit{extended metric} $d$ on a set $X$ is a function $d\colon X\times X\to\RR_{\ge0}$ satisfying (a)--(d) of \autoref{defi:metric}. We call $(X,d)$ an \textit{extended metric space}.
\end{definition}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Triangle inequality: we borrow from \href{https://terrytao.wordpress.com/2009/01/09/245b-notes-3-lp-spaces/}{here}. Given $f,g\in C([0,1])$, for psychological reasons we will assume that $f$ and $g$ are nonzero (else this is clear); then $\norm f,\norm g\ne0$, so we may scale everything so that $\norm f+\norm g$. In fact, we may again use scaling to find $a,b\in V$ such that
