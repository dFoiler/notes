...
\end{example}
Anyway, the point is that we may classify Lie algebras $\mf a$ over a field $K$ of characteristic zero by classifying the Lie algebras $\mf a\otimes_K\overline K$ over $\overline K$ (which is split!) and then classifying the Lie algebras that base-change to the split form. It will turn out that the Lie algebras $\mf a$ over $K$ with $\mf a\otimes_KL=\mf g_D$ are given by $\mathrm H^1(\op{Gal}(L/K);\op{Aut}\mf g_D)$. Let's recall what $\mathrm H^1$ means for our purposes.
\begin{definition}
