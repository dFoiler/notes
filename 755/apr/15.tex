...
\subsection{Maximal Abelian Subspaces}
We need to start by classifying maximal abelian subspaces of $\mf p$.
\begin{proposition} \label{prop:maximal-abelian-subspace}
...
\begin{proof}
	...
	\begin{listalph}
		...
		\item Choose maximal abelian subspaces $\mf a$ and $\mf a'$ of $\mf p_\theta$, and choose a generic $p\in\mc a'$ so that $\mf a'$ is the centralizer of $p$. By (c), there is $k\in K^c$ with $\op{Ad}_kp\in\mf a$. Then $\op{Ad}_k$ sends the centralizer of $p$ to the centralizer of $\op{Ad}_kp$, so it maps $\mf a'$ into $\mf a$. In other words, $\op{Ad}_k\mf a'\subseteq\mf a$. For example, this implies that $\dim\mf a'\le\dim\mf a$; reversing the argument shows that the dimensions are equal, so we conclude that $\op{Ad}_k\mf a'=\mf a$.
		\qedhere
	\end{listalph}
\end{proof}
