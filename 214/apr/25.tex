...
Today we talk about differential forms.
\begin{remark}
...
\end{example}
\begin{example} \label{ex:change-coords-n-form}
...
\end{example}
\begin{remark}
	As a multilinear map, we can compute that
	\begin{align*}
		d\omega(X_1,\ldots,X_{k+1}) &= \sum_{i=1}^{k+1}(-1)^{i+1}X_i(\omega(X_1,\ldots,\widehat{X_i},\ldots,X_{k+1})) \\
		&\qquad+\sum_{i<j}(-1)^{i+j}\omega([X_i,X_j],X_i,\ldots,\widehat{X_i},\ldots,\widehat{X_j},\ldots,X_{k+1}).
	\end{align*}
\end{remark}
\begin{remark}[Cartan's magic formula]
	One can compute that $\mc L_V\omega=V\lrcorner d\omega+d(V\lrcorner\omega)$.
\end{remark}
\begin{remark}
