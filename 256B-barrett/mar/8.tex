...
\begin{example}
	Notably, in our case of $S=A[x_0,\ldots,x_n]$, we see that $\OO(n)$ is a line bundle by checking affine-locally, where $\OO(n)$ just looks like $\Spec k[x_{0/i},\ldots,x_{n/i}]/(x_{i/i}-1)$ on the distinguished open subscheme $D_+(x_i)$.
\end{example}

Our goal is to compute the cohomology of these quasicoherent sheaves $\OO(n)$. Here is a start.
\begin{proposition} \label{prop:h0-proj-space}
