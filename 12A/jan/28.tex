...
\begin{definition}
	...
	\begin{listalph}
		\item $\varphi_k$ may be an atopic formula.
...
\begin{example}
	...
	\end{itemize}
	This is a contruction sequence for $(\lnot p_1\to(p_2\lor p_3))$.
\end{example}
The point is that constructon sequences let us define formulae.
...
\begin{example}
	The natural numbers $\NN$ are minimal with respect to $0\in\NN$ and closure under $+1$; its existence can be gauranteed via the same intersection idea. This means that any set containing $0$ and closed under $+1$ will contain $\NN$.
\end{example}
\begin{remark}
