...
\begin{remark}
	...
	\[V(IJ)\supseteq V(I\cap J)\supseteq V(\rad(IJ)),\]
...
\subsection{Some Affine Embeddings}
...
\begin{proposition}
	The mapping $\op{Spec}$ sending rings $A$ to topological spaces $\op{Spec}A$ and ring homomorphisms $\varphi\colon A\to B$ to continuous maps $\op{Spec}\varphi=\varphi^{-1}$ assembles into a functor ${\op{Spec}A}\colon\op{Ring}\opp\to\op{Top}$.
\end{proposition}
\begin{proof}
...
\end{example}
\begin{remark}
	Not every open set is of the form $D(f)$. For example,
	\[\AA^2_k\setminus\{(0,0)\}\subseteq\AA^2_k\]
	is an open set not in that form.
\end{remark}
