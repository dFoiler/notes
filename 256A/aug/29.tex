...
We are now ready to define the structure sheaf.  We use \autoref{prop:sheaffrombase}. Note that $D(f)\subseteq D(g)$ implies $V(g)\subseteq V(f)$, so $I(V(g))\supseteq I(V(f))$, so $f\in\rad g$, so $f^n=ag$ for some positive integer $n$. This implies that $g$ is a unit in $A_f$, so there is a natural restriction map
