...
\begin{lemma} \label{lem:moniciszeroker}
	Let $\mc C$ be an additive category with kernels, and fix a morphism $\varphi\colon\mc F\to\mc G$. Then $\varphi$ is monic if and only if $\ker\varphi$ vanishes.
\end{lemma}
\begin{proof}
	This is purely categorical; let $\iota\colon(\ker\varphi)\to\mc F$ be the kernel map. In one direction, suppose that $\ker\varphi$ vanishes. To show $\varphi$ is monic, write down
	...
	\end{tikzcd}\]
	with $\varphi\circ\psi_1=\varphi\circ\psi_2$, we need to show $\psi_1=\psi_2$. Well, $\psi\coloneqq\psi_1-\psi_2$ has $\varphi\circ\psi=0$, so our kernel promises a unique map $\overline\psi\colon\mc H\to(\ker\varphi)$ with $\psi=\iota\circ\overline\psi$. However, $\ker\varphi$ is the zero object, so we conclude $\psi=0$.

	In the other direction, suppose $\varphi$ is monic, and we show that the zero object $\mc Z$ satisfies the universal property of the kernel. Well, fix an object $\mc H$ with a map $\psi\colon\mc H\to\mc F$ such that $\varphi\circ\psi=0$. Then we need a unique map $\overline\psi$ making
	...
	\end{tikzcd}\]
	commute. Well, the map $\mc H\to\mc Z$ is certainly unique because $\mc Z$ is terminal. Additionally, we note that the zero map $\mc H\to\mc Z$ does indeed make the diagram commute: $\varphi\circ\psi=0=\varphi\circ0$ forces $\psi=0$, so $\psi$ is the zero map.
\end{proof}
...
\begin{proof}
	...
	\begin{enumerate}[label=(\alph*')]
		...
		\item $\ker\varphi_p$ vanishes for each $p\in X$. By \autoref{lem:kerstalk}, this is equivalent to $(\ker\varphi_p)$ vanishing for each $p\in X$.
	\end{enumerate}
...
\begin{proof}
	...
	By symmetry, we see $\varphi_p\circ\psi_p=\id{\mc G_p}$ as well, finishing.
	...
	We claim that $f_p$ is a set of compatible germs, which gives rise to a section $f\in\mc F(U)$ by \autoref{prop:injonstalks}. Indeed, for each $p\in U$, we can find $U_p\subseteq U$ small enough so that we can find $\widetilde f_p\in\mc F(U_p)$ with $\widetilde f_p|_p=f_p$. Continuing, we have
	\[\varphi_p(f_p)=[(U_p,\varphi_{U_p}(\widetilde f_p))]=g|_p,\]
	so we can shrink $U_p$ further (and restrict $\widetilde f_p$ accordingly) so that $\varphi_{U_p}(f_p)=g|_{U_p}$ as well.

	Now, for any $q\in U_p$, we see that
	\[\varphi_q(\widetilde f_p|_q)=\varphi_q(\widetilde f_p)|_q=\varphi_{U_p}(\widetilde f_p)|_q=g|_{U_p}|_q=g|_q=\varphi_q(\widetilde f_q),\]
	so we conclude $\widetilde f_p|_q=\widetilde f_q$ because $\varphi_q$ is an isomorphism. Thus, $(f_p)_{p\in U}$ is a system of compatible germs and therefore lifts to some $f\in\mc F(U)$ with $f|_p=f_p$ everywhere. Thus, $\varphi_U(f)|_p=\varphi_p(f|_p)=\varphi_p(f_p)=g|_p$ for each $p\in X$, so \autoref{prop:injonstalks} gives $\varphi_U(f)=g$.
\end{proof}
\begin{remark}
