...
\begin{lemma}
	Fix a morphism $\varphi\colon\mc F\to\mc G$ of sheaves on $X$. Then, for any $p\in X$, we have
...
\end{proposition}
\begin{proof}
	Note (a) is equivalent to $\ker\varphi$ vanishing by \autoref{lem:moniciszeroker}, which just means that $\ker\varphi_U=0$ always. However, $\ker\varphi_U=0$ is equivalent to $\varphi_U$ being monic by using \autoref{lem:moniciszeroker} again. Thus, (a) is equivalent to (b). We omit the proof of the equivalence of (c).
	% Use the embedding
	% \[\mc F(U)\into\prod_{p\in U}\mc F_p\]
	% for each $U\subseteq X$.
\end{proof}
\begin{remark}
	Technically, we only need to know that $\mc F$ is a sheaf for the above argument.
\end{remark}
