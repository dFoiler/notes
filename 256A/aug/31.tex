...
\begin{proposition} \label{prop:isoonstalks}
	Fix a morphism of presheaves $\varphi\colon\mc F\to\mc G$ such that $\mc F$ is a sheaf. The following are equivalent.
	\begin{listalph}
...
\begin{proof}
	...
	Thus, the hard direction is showing that $\varphi_p$ being an isomorphism for all $p\in X$ promises that $\varphi_U$ is an isomorphism for each $U\subseteq X$. Injectivity is easier: by \autoref{prop:stalkinjection} and \autoref{prop:stalkmorphism}, we see the diagram
	% https://q.uiver.app/?q=&macro_url=https%3A%2F%2Fraw.githubusercontent.com%2FdFoiler%2Fnotes%2Fmaster%2Fnir.tex
	\[\begin{tikzcd}
		{\mc F(U)} & {\displaystyle\prod_{p\in U}\mc F_p} & f & {(f|_p)_{p\in U}} \\
		{\mc G(U)} & {\displaystyle\prod_{p\in U}\mc G_p} & {\varphi_Uf} & {((\varphi_Uf)|_p)_{p\in U}}
	...
	\end{tikzcd}\]
	commutes. However, the top arrow is injective by \autoref{prop:stalkinjection}, and the right arrow is injective because each of the $\varphi_p$ is injective, so the product morphism is also injective. It follows that the left arrow must be injective, which is what we wanted.
	
	We now focus on showing $\varphi_U$ is surjective. Well, for any $g\in\mc G(U)$, we get a system of compatible germs $(g|_p)_{p\in U}$ (by \autoref{prop:injonstalks}), so because $\varphi_p$ is an isomorphism, we may set
