...
\begin{definition}[Generic point]
	Fix a topological space $X$. A point $\xi\in X$ is a \textit{generic point} if and only if the only point specializing to $\overline{\{\xi\}}$ is $\xi$.
\end{definition}
\begin{remark}
	Now let $(X,\OO_X)$ be a scheme. If $Z\subseteq X$ is an irreducible closed subset, then there is a unique generic point $\xi\in X$ such that $\overline{\{\xi\}}=Z$.\todo{Show this.}
\end{remark}
As such, we have upgraded \autoref{lem:affineirredcomps} to a bijection between generic points of $X$ and irreducible components of $X$ in a general scheme.
