...
upon taking the limit (in the category of \'etale sheaves) again. Now, using the top horizontal map in the above diagram (which is an isomorphism of schemes), we gain a canonical isomorphism $F^*_{\mc G}\colon F^*_{X_{\ov k}}\mc G\to\mc G$ of sheaves by taking the limit, so one gets a composite
\[\mc G\to F_{X_{\ov k}}^*\mc G\to\mc G.\]
This allows to define the $L$-function for $\mc G$.
\begin{definition}
	Fix a $\ov\QQ_\ell$-sheaf $\mc G$, as above. Then we define
	\[L(X,\mc G,T)\coloneqq\prod_{\text{closed }x\in X}\det\left(1-t^{\deg x}F_x,\mc G_{\ov x}\right).\]
\end{definition}
We will say more about this definition on Thursday.
