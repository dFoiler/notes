...
\begin{itemize}
	...
	\[\begin{tikzcd}
		...
		\arrow["\alpha", shorten <=3pt, shorten >=3pt, Rightarrow, from=0, to=1]
	\end{tikzcd}\]
	We define $H'\alpha:H'F\Rightarrow H'G$ by $(H'\alpha)_d:=H'\alpha_d$. This is a natural transfoormation because we can pick up some morphism $f:c\to d$ in $\mathcal D$ and apply $H'$ to the naturality diagram for $\alpha$, giving the following commutative naturality square.
...
\begin{itemize}
	...
	\[\begin{tikzcd}
		...
		\arrow["\alpha", shorten <=3pt, shorten >=3pt, Rightarrow, from=0, to=1]
	\end{tikzcd}\]
	In total, we see that $(G'\alpha)(\beta F):F'F\Rightarrow G'G$. Note ths is a natural transformation by vertical composition!
	\item We start by whiskering on the right and then whisker on the left. So we start by onting we have $F'\alpha:F'F\Rightarrow F'G$ by whiserking along the following diagram.
...
\[\begin{tikzcd}
	...
	\arrow["{H'\beta}"', from=3-2, to=3-3]
\end{tikzcd}\]
The top leg is $\alpha'*\alpha$, and the right leg is $\beta'*\beta$, so this total red path comes out to $(\beta'*\beta)(\alpha'*\alpha)$.

So comparing our two red paths, we see that
\[(\beta'\alpha')*(\beta\alpha)=(\beta'*\beta)(\alpha'\alpha),\]
which is what we wanetd.