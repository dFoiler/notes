...
\begin{proof}
	...
	\begin{itemize}
		\item The functor $F$ is fully faithful because (based) lienar transformations $(V,\mathcal B)\to(V',\mathcal B')$ are in bijective correspondence with matrices in $k^{\dim V'\times\dim V}$, which is exactly $\op{Mor}_{\op{Mat}_k}$
...
\end{remark}
Let's see an example of the power of abstracting diagams.
...
\end{example}
\begin{example}
	We can think about triangles as non-injetive images of squares, as follows.
