...
\begin{definition}[Small, locally small]
	Fix $\mathcal C$ a category. Then $\mathcal C$ is \textit{small} if and only if $\op{Mor}\mathcal C$ is a set. Alternatively, $\mathcal C$ is \textit{locally small} if and only if $\op{Mor}(x,y)$ is a set.
\end{definition}
\begin{example}
