...
\begin{notation}
	For each $n$, we set $G_n\coloneqq\OO_n^\times$, where $\OO_n\coloneqq W(k)\langle S\rangle/\left(S^n-p,Sa-a^\sigma S\right)$. We also define $G_{h,h-1}\coloneqq(G_h\times G_{h-1})\rtimes\op{Gal}(k/\FF_p)$.
\end{notation}
...
\end{remark}
We can use this to define the determinant map.
\begin{definition}
	We define the map $\det\colon G_h\to\ZZ_p^\times$ as sending the automorphism $\sigma$ of $\Gamma_h$ to the corresponding scalar which is the automorphism of the determinant of the corresponding Dieudonn\'e module. Its kernel is denoted $G_h^1$.
\end{definition}
...
% We would like to study $\op{BC}(-1/(h-1))^*$, where $(-)^*$ means that we want the extension to be nontrivial. The Banach--Colmez spaces have well-understood cohomology, so our next task is to understand the cohomology in the punctured setting.
% \begin{remark}
