...
\begin{theorem} \label{thm:thick-finite-subcat}
	For each $n\ge0$, the category $\mc C_n$ of finite spectra of type at least $n$ is a thick subcategory of $\mathbb S_{(p)}$-modules. Conversely, any thick subcategory containing only $\mathbb S_{(p)}$-modules takes the form $\mc C_n$ for some $n\ge0$.
\end{theorem}
\begin{example}
	It would follow from \Cref{ex:s-eta-type-zero} and \Cref{thm:thick-finite-subcat} that the thick subcategory generated by $\mathbb S/\eta$ should have all finite spectra. Well, $\mathbb S/\eta^2$ is an extension of $\mathbb S/\eta$ by itself, and one can similarly construct $\mathbb S/\eta^3$ and $\mathbb S/\eta^4$. But $\eta^4=0$, so $\mathbb S/\eta^4=\mathbb S\oplus\Sigma^5\mathbb S$, which contains $\mathbb S$ as a retract. Thus, we get all perfect $\mathbb S$-modules!
\end{example}
