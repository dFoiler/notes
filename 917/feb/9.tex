...
\end{remark}
Next class, we will say something about formal groups.
\begin{remark}
	There is a functor $S\colon\mathrm{Vec}_\CC\to\mathrm{Top}_*$ of ordinary category given by taking the one-point compactification. It satisfies $S(V\oplus W)=SV\land SW$, so we get a map
	\[\bigsqcup_{n\ge0}\mathrm {BU}(n)\to\mathrm{Top}_*\to\mathrm{Spaces}_*.\]
	This colimit is related to $\mathrm{MU}$, and it inherits the structure of an $\mathbb E_\infty$-ring, and this is the only known reason why.
\end{remark}
