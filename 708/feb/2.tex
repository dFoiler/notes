...
\begin{restatable}[Cartier isomorphism]{theorem}{cartieriso} \label{thm:cartier-iso}
...
\end{definition}
\begin{example}
	Given a scheme $Y$ over $W(k)$, we can produce a formal scheme $\widehat Y$ with $\widehat Y_n\coloneqq Y\times_{W(k)}W_n(k)$ and the induced internal isomorphisms.
\end{example}
\begin{remark}
	If $X_1$ is affine, then all the nilpotent thickenings are affine, so we may say that the full formal scheme is affine.
\end{remark}
\begin{remark}
	We can even define $\widehat\Omega X^1_{\widetilde X}$.
\end{remark}
We can now describe crystalline cohomology.
\begin{theorem} \label{thm:crystalline-props}
	...
	\begin{listalph}
		...
		\item If $\widetilde X$ is a smooth formal scheme over $W(k)$ (meaning that $\widetilde X_n$ is smooth over $W_n(k)$ for all $n$), then the scheme $X\coloneqq\widetilde X\times_{W(k)}k$ has
		\[\mathrm R\Gamma_{\mathrm{cris}}(X/W(k))\cong\mathrm R\Gamma_{\mathrm{dR}}\left(X;\widehat\OO_{\widetilde X}\stackrel d\to\widehat\Omega^1_{\widetilde X}\stackrel d\to\cdots\right).\]
		This right-hand side can be thought of as $\mathrm R\Gamma_{\mathrm{dR}}(\widetilde X/W(k))$.
	\end{listalph}
\end{theorem}
\begin{remark}
