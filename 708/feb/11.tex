...
\begin{remark}
	...
	\[\mathrm H^n_{\mathrm{dR}}(X/k)=\bigoplus_{i+j=n}\mathrm H^i(X^{(1)};\Omega^j_{X^{(1)}}).\]
	It turns out that this decomposition remains true if $X$ can merely be lifted to $W_2(k)$, and it even depends on this choice of lift.
\end{remark}
...
\begin{example}
	...
	However, the left-hand formal scheme is $\AA^1\times\widehat{\mathbb G}_a$ (given by sending $(x,y)$ to $(x,x-y)$), and one can check that the cocycle condition corresponds to an equivariant structure. It follows that we are looking at quasicoherent sheaves on $\AA^1/\widehat{\mathbb G}_a$.
\end{example}
