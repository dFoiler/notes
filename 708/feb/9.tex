...
\begin{example}
	...
	For example, if $X$ is defined over $\FF_{q^r}$, then $X=X^{(r)}$, then we have an endomorphism $(F^*)^r$ of the complex $\mathrm R\Gamma_{\mathrm{cris}}(X/W(k))$. Here is an application: if $X$ is further proper, then $\mathrm H^n_{\mathrm{cris}}(X/W(\FF_q))$ is finitely generated over $W(\FF_q)$, and it turns out that
