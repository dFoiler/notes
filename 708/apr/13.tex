...
\end{definition}
\begin{remark}
	For motivation, we give a coordinate-free definition: for a $3$-dimensional vector space $E$ over $k$, we may view $\PP^2_k=\PP E$ as hyperplanes in $E$, and we note that homogeneous polynomials $s$ of degree $2$ are elements in $\op{Sym}^2E$.
\end{remark}
\begin{definition}[conic bundle]
	A \textit{conic bundle} over a scheme $B$ is a scheme $C\to B$ which is cut out in $\PP\mc E$, where $\mc E$ is a vector bundle of rank $3$ on $B$, by a nowhere vanishing section $s\in\Gamma\left(B;\op{Sym}^2\mc E\right)$.
\end{definition}
\begin{remark}
...
\begin{example}[Ch\^atelet surface] \label{ex:chat}
	Fix a field $k$ of characteristic not two, and choose a unit $a\in k^\times$ and a separable quartic $P(x)\in k[x]$ with homogenization $\widetilde P(x,w)\in k[x,w]$. Then we can define $U\colon y^2-az^2=P(x)$ in $\AA^3$, which completes to a conic bundle $X\to\PP^1$ as follows: let our line bundle be $\mc E\coloneqq\OO\oplus\OO\oplus\OO(2)$, and choose our section to be $s\coloneqq(1,-a,-\widetilde P)$, which then cuts out a diagonal conic bundle. Then $X$ contains $U$ as a dense open subscheme. The variety $X$ is called a Ch\^atelet surface.
\end{example}
...
\begin{remark} \label{rem:isok-infinity}
	The fiber over $x=\infty$ is the projective conic $y^2+z^2=-w^2$.
\end{remark}
...
\end{proof}
\begin{remark}
	Morally, what is going on here is that for any prime $p$ such that $X$ and $A$ both have good reduction at $p$, the map $\op{ev}_A\colon X(\QQ_p)\to\op{Br}\QQ_p$ is zero. It is only at the place $p=2$ where we have a chance of finding some nontrivial invariants.
\end{remark}
...
\end{proof}
\begin{remark}
	It remains a question where the magical quaternion algebra $A$ comes from. The idea is that $3-x^2$ and $x^2-2$ sum to $1$, so they are relatively prime. However, they multiply to a norm $y^2+z^2$, so one expects both of them to individually be norms. Thus, the quaternion algebra $A$ should be found to be trivial most of the time, with some notable problems at $2$.
\end{remark}
\begin{remark}
	It turns out that elements of $\op{Br}X$ coming from $\op{Br}\QQ$ never provide an obstruction to rational points. Instead, for Iskovskikh's surface $X$, one can use the techniques from \Cref{subsec:brauer-computations} to show that there is a split exact sequence
...
\end{remark}
\begin{remark}
	One does not expect the Brauer--Manin obstruction to be sharp. But Corwin and Schland proved that if $k$ is a totally real or quadratic imaginary number field and $X$ is a $k$-variety with no $k$-point, then there exists a Zariski open cover $\{X_i\}$ such that $X_i(\AA_k)^{\mathrm{Br}}$ is empty for every $i$.
	% However, if we pass to covers, then one can show that the obstruction is sharp. Explicitly, one needs to test the Brauer--Manin obstruction on all open covers $X=\bigcup_iX_i$, where the idea is that
...
\[X(k)\subseteq\bigcup_{\sigma\in\mathrm H^1(k;G)}f^\sigma(Y^\sigma(\AA_k)).\]
...
\begin{definition}[descent set]
	...
	\[X(\AA_k)^f\coloneqq\bigcup_{\sigma\in\mathrm H^1(k;G)}f^\sigma(Y^\sigma(\AA_k)).\]
