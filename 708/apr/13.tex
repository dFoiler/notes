...
\begin{definition}[descent set]
	...
	We define the \textit{descent set} $X(\AA_k)^{\mathrm{descent}}$ to be the intersection of $X(\AA_k)^f$ as $f$ varies over all $G$-torsors.
\end{definition}
\begin{remark}
	One can recover the weak Mordell--Weil theorem for an elliptic curve $E$ by taking $G=E[2]$.
\end{remark}
\begin{remark}
	It turns out that the Brauer--Manin obstruction is a special case of the descent obstruction for the groups $\op{PGL}_n$.
\end{remark}
