...
\end{example}
\begin{example}
	Lind and Reichardt in the 1940s produced some example of genus-$1$ curves where the Hasse principle fails, such as $3x^3+4y^2+5z^3=0$ (in $\PP^2_\QQ$) and a smooth projective model of $2y^2=1-17x^4$.
\end{example}
\begin{remark}
...
\end{remark}
\begin{example}
	Swinnerton-Dyer constructed some cubic surfaces where the Hasse principle fails, such as $5x^3+9y^3+10z^3+12w^3=0$ in $\PP^3_\QQ$.
\end{example}
...
Let's start with some algebra. Throughout, when $k$ is a field, we let $k^{\mathrm{sep}}$ be its separable closure, and $\overline k$ is its algebraic closure. For convenience, we also set $G_k\coloneqq\op{Gal}(k^{\mathrm{sep}}/k)$.
\begin{definition}
	An \textit{algebra} over a field $k$ is an associative unital ring with $k$-action.
\end{definition}
...
\end{remark}
\begin{remark}
	The collection $\op{Br}(k)$ is a group under $\otimes$ (which turns out to send central simple algebras to central simple algebras). The unit is $k$, and the inverse of $A$ is $A\opp$; for example, one needs to check that $A\otimes A\opp$ is isomorphic to a matrix algebra over $k$.
\end{remark}
\begin{remark}
	A field extension $k\into\ell$ induces a group homomorphism $\op{Br}(k)\to\op{Br}(\ell)$ given by $-\otimes_k\ell$. Notably, $-\otimes_k\ell$ sends central simple algebras to central simple algebras by \Cref{prop:csa-grab-bag}(b), and it preserves equivalence by the definition given in \Cref{rem:better-equiv}.
\end{remark}
...
\end{proposition}
\begin{proof}
	For each $n$, \Cref{prop:csa-grab-bag} central simple algebras of dimension $n^2$ are Galois twists of $M_n(k^{\mathrm{sep}})$, so they are classified by the pointed set $\mathrm H^1(k;\op{Aut}M_n(k^{\mathrm{sep}}))$. By the Skolem--Noether theorem, all (algebra) automorphisms of $M_n(k^{\mathrm{sep}})$ are inner, so this automorphism group is $\op{PGL}_n(k^{\mathrm{sep}})$. To compute this cohomology group, we note that there is a short exact sequence
	...
	\[\mathrm H^1(k;\mathbb G_m)\to\mathrm H^1(k;\mathrm{GL}_n)\to\mathrm H^1(k;\mathrm{PGL}_n)\to\mathrm H^2(k;\mathbb G_m)\]
	of pointed sets. The left two groups vanish by (a version of) Hilbert's theorem 90. (This is basically the statement that all $\op{GL}_n$-bundles on $\Spec k$ are trivial, which is true because all rank-$n$ modules over $k$ are free.) It turns out that this is enough to produce an injection $\mathrm H^1(k;\mathrm{PGL}_n)\to\mathrm H^2(k;\mathbb G_m)$.
...
\begin{proof}
	...
	\[\mathrm H^2(k;\mathbb G_m)\to\mathrm H^2(k;\mu_n)\to\mathrm H^2(k;\mathbb G_m)\stackrel n\to\mathrm H^(k;\mathbb G_m),\]
...
\begin{example} \label{ex:quaternion-by-cup}
	There is a canonical cup product
...
\end{example}
\begin{example}
	By Wedderburn's theorem, all finite division algebras are fields, so there is only one finite central division algebras over $\FF_q$ (namely, $\FF_q$). Thus, $\op{Br}\FF_q=0$.
\end{example}
\begin{example}
...
\end{example}
\begin{example}
	A direct calculation in Galois cohomology, one has that $\op{Br}\RR=\frac12\ZZ/\ZZ$, where $\RR$ goes to $0$, and $\HH$ goes to $\frac12$. This is equivalent to \Cref{ex:real-csa}.
\end{example}
\begin{example}
...
\end{example}
\begin{remark}
	It turns out that the short exact sequence
	\[0\to\op{Br}\QQ\to\bigoplus_v\op{Br}\QQ_v\to\QQ/\ZZ\to0\]
	encodes quadratic reciprocity, if we apply to the quaternion algebra $H_{p,q}$ where $p$ and $q$ are odd primes.
\end{remark}
