...
\begin{theorem}[de Saint Germain--Huang--Lu]
...
\end{theorem}
\begin{proof}[Idea]
	One can glue the tori together with the ``boundary'' cut out by $x_i=0$. This gluing process allows one to glue together the holomorphic $n$-forms
	\[\Omega_\Sigma=\frac{dx_1}{x_1}\land\cdots\land\frac{dx_n}{x_n}\]
	on the cluster variety. Then $\Omega_\Sigma$ vanishes nowhere on the interior. The adjective ``log'' is present in the theorem because the resulting $n$-form $\Omega$ may have logarithmic poles on the boundary.
\end{proof}
...
It will help to have the following automorphism. The following is due to Kontsevich--Soibelman, Keller, Fock--Goncharov, Gross--Hacking--Keel, and Fomin--Zelevinsky.
...
\end{theorem}
\begin{remark}
	This $\tau$ is given by horizontally translating a frieze one column. From here, one still must check that $\tau$ descends to a regular automorphism on the cluster variety, preserves volumes, and is periodic. None of these checks are particularly easy. %which Thus, the existence of the automorphism and the fact that it preserves volumes is not hard to check. The difficulty is the period; for example, for $\mf{sl}_3$, this follows from the work of Gauss.
\end{remark}
\begin{remark}
...
\end{theorem}
\begin{proof}[Sketch]
	Let's begin with the case where $\Delta$ is infinite. Here are two arguments.
	\begin{itemize}
		\item Following Morier--Genoud, one can construct many maps $\mc A\to\ZZ$, basically by sending one collection of cluster variables to $1$. There are infinitely many sets of cluster variables, so this produces infinitely many friezes.
...
\begin{example}
	...
	Such equations appear in the Mordell--Schinzel conjecture, which dates back to a 1952 paper of Mordell. Using our developed theory, we can see that this equation has $5$ points if $ab=1$, $6$ points if $ab=2$, and $9$ points of $ab=9$, and there are infinitely many points if $ab\ge4$. More generally, if $G(x,y)$ has positive integral coefficients, we are able to determine when $xyz=G(x,y)$ should have infinitely many positive integral solutions. For example, there are infinitely many when $\deg_xG$ and $\deg_yG$ are both at least three and has non-vanishing constant term.
\end{example}
