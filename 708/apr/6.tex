...
\begin{definition}[Azumaya algebra]
	Fix a scheme $X$. Then an \textit{Azumaya algebra} $\mc A$ on $X$ is an $\mathcal O_X$-algebra that is \'etale locally isomorphic to a matrix algebra of positive rank. More precisely, there is an \'etale cover $\{U_i\}$ of $X$ and a tuple of positive integer ranks $\{r_i\}$ such that there are isomorphisms $\mc A|_{U_i}\cong M_{r_i}(\OO_{U_i})$ for each $i$.
	% \begin{listalph}
...
\end{definition}
\begin{remark}
	When $X=\Spec k$ for a field $k$, this definition is equivalent to that in \Cref{prop:csa-grab-bag}(ii).
\end{remark}
\begin{remark}
	If $X$ is connected, then it turns out that the rank of $\mc A$ is constant because $\mc A$ is locally free!
\end{remark}
...
\end{example}
We can now use Azumaya algebras to define a Brauer group, using a similar notion of equivalence.
\begin{definition}[Brauer group]
	Fix a scheme $X$. Then two Azumaya algebras $\mc A$ and $\mc A'$ are \textit{equivalent} if and only if there are locally free $\OO_X$-modules $\mc E$ and $\mc E'$ of finite positive rank everywhere on $X$ such that
...
\end{definition}
\begin{remark}
	Similarly to \Cref{rem:br-is-group}, the Brauer group is a group with operation given by the tensor product, and inverses given by the opposite Azumaya algebra.
\end{remark}
...
\begin{proposition} \label{prop:function-field-injective}
	Fix a regular integral Noetherian scheme $X$, and let $K(X)$ be the function field of $X$. Then the map $\op{Br}X\to\op{Br}K(X)$ is injective.
\end{proposition}
% \begin{proof}[Idea]
...
\end{example}
\begin{remark} \label{rem:br-k-sep-curve}
	In fact, $\op{Br}X$ is trivial also for any nice curve $X$ over a \textit{separably} closed field.
\end{remark}
% We can even go back and do some calculations for fields.
\begin{proposition} \label{prop:br-dvr}
	Fix a complete discrete valuation ring $R$ with residue field $k$. Then the natural map $\op{Br}R\to\op{Br}k$ is an isomorphism.
\end{proposition}
\begin{proof}
...
\end{proof}
\begin{corollary} \label{cor:brauer-les}
