...
\end{definition}
\begin{example}
	The Parthenon featured a frieze, which was some horizontally periodic geometric pattern.
\end{example}
This definition is slightly too classical.
\begin{definition}[frieze]
...
\begin{theorem}[Gauss~1866]
	All friezes with two rows is $5$-periodic.
\end{theorem}
\begin{remark}
...
\end{definition}
\begin{example}
	Take $\Delta=A_n$ recovers a frieze with $n$ rows.
\end{example}
\begin{example}
...
\begin{theorem}[Assem--Reataver--Smith]
...
\end{theorem}
\begin{remark}
	We will say basically nothing about the proof, but we do say that many of the arguments here find bijections between these friezes and some kind of polygon triangularizations, perhaps with punctures and so on.
\end{remark}
\begin{remark}
...
We are now allowed to define cluster algebras. The most general definition is technical, so we will content ourselves with the following notion.
\begin{definition}
	...
	\begin{enumerate}
		...
		\[x_1'\coloneqq\frac{\prod_{\substack{1\le i\le n\\b_{i1}<0}}x_i^{\left|b_{i1}\right|}+\prod_{\substack{1\le i\le n\\b_{i1}>0}}x_i^{\left|b_{i1}\right|}}{x_1}.\]
