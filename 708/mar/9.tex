...
\end{remark}
Wall would classify conjugacy classes for classical groups in characteristic $2$. For good primes, the finiteness was proved for exceptional groups by Richardson (though without classification). Students of Iwahori would classify these conjugacy classes for types $G_2$ and $F_4$. Lusztig (based on work with Deligne) showed finiteness in full generality shortly thereafter, which was new for the exceptional types $E$, and classification followed in types $E$ by Mizuno.
...
\end{notation}
\begin{remark}
	The set $\mc D_G/G$ is independent of the field (it only depends on root datum). The $H^{\overline\omega}$ contains a unique dense unipotent class, so the moral is that there is an embedding from unipotent classes of $G$ over $\CC$ to unipotent classes of $G$ over $k$.
\end{remark}

\subsection{The Springer Resolution}
We will want to know a little bit about the Springer resolution.
\begin{notation}
	Fix a connected reductive group $G$ over an algebraically closed field $k$. Then $\mc B$ is the flag variety of Borel subgroups, and we define $W$ as the set of $G$-orbits on $\mc B\times\mc B$.
\end{notation}
% \begin{remark}
...
\begin{definition}[Springer resolution]
	...
	where $\widetilde{\mc B}\coloneqq\{(g,B)\in G\times\mc B:g\in B\}$. We let $\widetilde{\mc B}_{\mathrm{rs}}$ denote the pre-image of the regular semisimple locus of $G$.
\end{definition}
\begin{remark}
	The Springer resolution is generically $W$-to-$1$. Indeed, $\widetilde{\mc B}_{\mathrm{rs}}\to\mc B$ is an \'etale covering with fibers given by $W$. We then define an embedding $W\to\op{Aut}(\mc B_{\mathrm{rs}})$ by sending $w\in W$ to the automorphism $B\mapsto B'$, where $(B,B')\in w$. This defines the group structure of $W$ (as a subgroup of $\op{Aut}(\mc B_{\mathrm{rs}})$.
	%Thus, one has an embedding $W\to\op{Aut}\widetilde{\mc B}_{\mathrm{rs}}$, which turns out to be an embedding of groups.
\end{remark}
...
\end{notation}
\begin{remark}[Springer correspondence]
	The action of $W$ on $\mc B$ produces an action on $\mathrm H^*(\mc B;\overline\QQ_\ell)$. One can show that $V_g$ is a representation of $W$, and it turns out to be irreducible. In terms of homology, it is equivalent to ask for the image of this map (in top-degree of the right-hand homology group) to be irreducible.
\end{remark}
...
\end{example}
\begin{example}
	We work with $G\coloneqq\op{Sp}_4$ over $\CC$, and again let $G_d\subseteq G$ be the subset of those $g\in G$ for which $\dim Z_G(g)=d$. The $G_d$s turn out to not be the strata: indeed, $G_4$ splits into $G_4'\sqcup G_4''$, where the former is the conjugacy class of $\op{diag}(1,1,-1,-1)$, and the latter corresponds to a unipotent conjugacy class in characteristic $2$. But up to this splitting, we have indeed recovered our strata. Notably, $G_4'$ contains no unipotent class in characteristic zero. We are asked to show that these strata have smooth irreducible components on the homework, but it may be too difficult.
\end{example}
\begin{remark}
