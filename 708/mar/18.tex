...
\begin{proof}
	...
	\end{notation}
	\begin{remark}
		Note that $H_G(g)$ is the (connected component of a) Levi subgroup of some parabolic because it is the centralizer of a torus.
	\end{remark}
	\begin{definition}[piece]
		Let $G$ be a reductive group. For any Levi subgroup $L$ of a parabolic, we may let $S_1$ be the pre-image of an isolated class in $L/Z^\circ(L)$. (We will not define what it means to be isolated, but we will say that a semisimple class is isolated if it has the same rank as the full group.) We can then define $Y_{L,S_1}$ to be the union of all conjugacy classes in $G$ which meet those $g\in S_1$ and having $H_G(g)=L$. It turns out that there are only finitely many pairs $(L,S_1)$ up to conjugacy, and these $Y_{L,S_1}$s are the pieces of $G$.
	\end{definition}
	\begin{remark}
		Lusztig has shown that the closure of a piece is a union of pieces.
	\end{remark}
	Now, Carnovale shows that a stratum $\Sigma$ is a union of pieces. In fact, for any piece $J\subseteq\Sigma$, find $m$ with $\Sigma\subseteq G_m$, and then one can show that $\overline J\cap G_m\subseteq\Sigma$. It follows that $\Sigma$ is the union of these locally closed subsets $\overline J\cap G_m$, so $\Sigma$ is locally closed.
\end{proof}
\begin{remark}
