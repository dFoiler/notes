...
\begin{definition}
	...
	\[\mc B_{\gamma,w}\coloneqq\left\{(g,B)\in\gamma\times\mc B_p:\left(B,gBg^{-1}\right)\text{ are in relative position }w\right\}.\]
...
\end{theorem}
\Cref{thm:weyl-to-unip} was settled by a complicated argument for classical types and using a computer for exceptional types.
\begin{remark}
...
\end{example}
\begin{remark}
	In fact, in characteristic two, one finds that the reductive part of the centralizer of a unipotent element corresponding to an elliptic Weyl group element is a product of symplectic groups, and it kind of explains how one finds the sizes of the fibers. This feature works in general, not just for $E_8$.
	% centralizer of a UNIPOTENT ELEMENT corresponding to an elliptic Weyl group element is a PRODUCT OF symplectic groups
\end{remark}
...
\begin{example}
	...
	parameterize those elements in $\op{Irr}_*W$ corresponding to an elliptic conjugacy class in $W$.
\end{example}
