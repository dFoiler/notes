...
\end{definition}
\begin{remark}
	Having the same characteristic is an equivalence relation. Symmetry has little content, and one can check that $R\otimes_{\mathbb S}R\ne0$. For the transitivity, if $R_1\otimes_{\mathbb S}R_2$ and $R_2\otimes_{\mathbb S}R_3$ are both nonzero, then one notes that $R_1\otimes_{\mathbb S}R_2$ is a free module over $R_2$, so
...
\subsection{New Ring from Old Ones}
