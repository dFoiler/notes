...
\begin{defihelper}[$\mathbb E_1$-space] \nirindex{E1 space@$\mathbb E_1$-space}
	Given an $\infty$-groupoid $X$, an $\mathbb E_1$-structure on $X$ is a functor $M\colon\Delta\opp\to\mathrm{Spaces}$ such that $M_0=\Delta^0$ and $M_1=X$ and the injections $[1]\to[n]$ (given by $i\le i+1$) produce maps $M_n\to M_1^n$ which is an equivalence. In this situation, we may say that $M$ satisfies the ``Segal conditions.''
\end{defihelper}
\begin{remark}
