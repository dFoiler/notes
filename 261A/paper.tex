...
\end{definition}
\begin{remark}
	\Cref{prop:haar-unique} explains that left-invariant and right-invariant differential forms both form one-dimensional vector spaces, so $G$ is unimodular if and only if these are the same vector subspace of $\Gamma(G,\land^mT^*G)$.
\end{remark}
