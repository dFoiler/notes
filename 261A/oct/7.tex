...
\begin{proof}
	...
	\begin{listalph}
		...
		\begin{align*}
			...
			&= \chi_V(T)\chi_W(T),
		\end{align*}
		\item Let $\{v_1,\ldots,v_k\}$ be an eigenbasis for the operator $\rho_V(h)\colon V\to V$ with eigenvalues $\{\lambda_1,\ldots,\lambda_k\}$. Then we claim that the dual basis $\left\{v_1^\lor,\ldots,v_k^\lor\right\}$ is an eigenbasis for $\rho_{V^\lor}(h)$: for any $v_i^\lor$ and $v_j$, we see
		\begin{align*}
...
\end{definition}
\begin{remark}
	We won't bother to check that $T\mf g$ is in fact an associative $F$-algebra, but we do note that the multiplication is $F$-linear automatically by the construction.
\end{remark}
\begin{example}
...
\end{definition}
\begin{remark} \label{rem:general-ug}
	Technically speaking, the construction of $U\mf g$ did not require that $[-,-]$ be a Lie bracket. In particular, one can imagine constructing a version of $U\mf g$ for any bilinear form $[-,-]$ on a vector space $\mf g$ by taking the quotient of $T\mf g$ by the two-sided ideal generated by
	\[\{X\otimes Y-Y\otimes X-[X,Y]:X,Y\in\mf g\}.\]
\end{remark}
...
\begin{lemma} \label{lem:ad-action-ug}
	Fix a Lie algebra $\mf g$ over a field $F$. For $A\mf g\in\{T\mf g,U\mf g,S\mf g\}$, there are unique Lie algebra morphisms $\op{ad}_\bullet\colon\mf g\to\mf{gl}(A\mf g)$ (with infinite-dimensional target!) satisfying the following.
	\begin{itemize}
