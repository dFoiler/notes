...
\end{proof}
\begin{remark}
	This proof works fine as long as the base field is uncountable. If we want to work over $\ov\QQ$ or number fields, then a little trickery is required. For example, one can embed $\ov\QQ$ into $\CC$, and then the second step provides a complex eigenvalue for $T$, which can be checked to come down to $\ov\QQ$ because $T$ is defined over $\ov\QQ$.
\end{remark}
% K = Kbar
...
\end{idea}
Here's an example, which uses the notion of compactness.
\begin{proposition}
