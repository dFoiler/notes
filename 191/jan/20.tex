...
\begin{lemma}[Differentiation under the integral sign] \label{lem:diff-under-sign}
	...
	%  and satisfies
	% \[F'(s)=\int_\RR\frac{\del f(s,t)}{\del s}\,dt.\]
\end{lemma}
\begin{proof}
	% Fix some $z\in U$, and we show the result at $z$. Because $U$ is open, we may find $r>0$ such that $z\in B(z,r)$. Now, for any $z'\in B(z,r)$, we let $\gamma\colon[0,1]\to B(z,r)$ denote the straight line from $z$ to $z'$. Thus, the Fundamental theorem of calculus grants
	% \[\int_\RR\left(\int_\gamma\frac{\del f(s,t)}{\del s}\,ds\right)dt=\int_\RR\big(f(z',t)-f(z,t)\big)\,dt=F(z')-F(z).\]
	% We would like to exchange these two integrals, so we use Fubini's theorem to check
	% \[\int_\RR\left(\int_\gamma\left|\frac{\del f(s,t)}{\del s}\right|\,ds\right)dt\le\int_\RR\ell(\gamma)g(t)\,dt=\ell(\gamma)\int_\RR g(t)\,dt<\infty.\]
	% Thus, we may interchange our integrals, whereupon we see
	% \[\frac{F(z')-F(z)}{z'-z}=\frac1{z'-z}\int_\gamma\left(\int_\RR\frac{\del f(s,t)}{\del s}\,dt\right)ds\]
	% for any $z'\ne z$. For brevity, we define $F_0(s)\coloneqq\int_\RR(\del f(s,t)/\del s)\,dt$, which we know absolutely converges by the above argument.
	% Now, let $\{z_n\}_{n\in\NN}\subseteq B(z,r)$ denote any sequence distinct from $z$ but approaching $z$.
...
\begin{proof}
	...
	by \Cref{rem:annoying-log-power-ints}. Thus, \Cref{lem:diff-under-sign} implies that $I(s)$ is holomorphic on $\{s:\op{Re}s>\sigma\}$, finishing the proof.
