...
\begin{proof}
	...
	The point is to compute the Fourier series of $F\colon\RR\to\CC$. We run a few quick checks on $F$.
	\begin{itemize}
	...
	\end{itemize}
	The next step is compute the Fourier coefficients of $F$, which for some $n\in\ZZ$ is
	...
	We would like to build the Fourier series using \Cref{thm:fourier-series}, but for this we must show that $S_F$ converges absolutely and uniformly. Well, by \Cref{lem:fourier-checks}, we see that $n\ne0$ have
	...
	which does vanish as $N\to\infty$. This gives our absolute and uniform convergence, so \Cref{thm:fourier-series} tells us
