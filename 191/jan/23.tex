...
\begin{proof}
	...
	\begin{enumerate}
		...
		We now use the Mellin transform $\mc M\varphi$. Indeed, note that $\varphi$ is rapidly decaying at $0$ and $\infty$ because $\varphi$ actually vanishes for $x<0.9$ and $x>2.1$. Now, we use \Cref{thm:inv-mellin} to compute
		...
		However, by \Cref{prop:bound-mellin-transform}, there is $C$ such that $|(\mc M\varphi)(s)|\le C(\op{Im}s)^{-4}$, so we bound
		\begin{align*}
