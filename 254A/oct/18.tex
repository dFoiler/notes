...
\begin{proof}
	...
	\begin{itemize}
		...
		In particular, $S^{-1}\mathcal O_K$ only has finitely many maximal ideals (it only has the ones lying above $(p$) and Dedekind, it is a principal ideal domain, and in particular, we see that $S^{-1}\mathcal O_L$ has an integral basis over $S^{-1}\mathcal O_K,$\todo{why} which we call $\{\omega_k\}_{k=1}^d,$ where $d\coloneqq \varphi\left(\ell^\nu\right).$ Letting $M$ be the matrix expressing the $(\zeta')^\bullet$ in terms of the $\omega_\bullet,$ we see that
