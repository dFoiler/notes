...
\begin{proof}
	...
	\end{enumerate}
\end{proof}
The above result is a bit unsatisfying because we have not determined which of $p=a^2+5b^2$ or $2p=a^2+5b^2$ is true. We will explain what is going on later on, specifically in \Cref{ex:full-primes-of-form-5}.
...
\begin{proof}
	...
	\end{enumerate}
\end{proof}
The benefit of working with a general inner product $\langle\cdot,\cdot\rangle$ is that we can now we can use \Cref{prop:gauss-reduce} to reduce more general quadratic forms instead of merely $\langle\cdot,\cdot\rangle$. For example, the inner product
\[\langle(x_1,y_1),(x_2,y_2)\rangle\coloneqq x_1x_2+5y_1y_2\]
is positive-definite, so we may do \Cref{prop:gauss-reduce}. For an example computation of this, we refer to \Cref{prob:explicit-primes-of-form-5}.
% \begin{example}
...
\begin{prob}[8 points] \label{prob:explicit-primes-of-form-5}
	...
	\begin{listalph}
		\item Use \Cref{prob:3-mod-4-square-root} to compute an integer $x$ such that $x^2\equiv-5\pmod p$.
		\item Using the proof of \Cref{prop:primes-of-form-5} and the algorithm of \Cref{prop:gauss-reduce}, compute a pair of integers $(a,b)$ such that either $p=a^2+5b^2$ or $2p=a^2+5b^2$.
	\end{listalph}
\end{prob}
\begin{prob}[0 points]
