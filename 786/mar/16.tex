...
\begin{proof}
	...
	\begin{itemize}
		...
		\item Discrete series: there are irreducible Verma modules, which start from a given positive integer $m\ge2$ and extend off infinitely in the positive direction; the weights are $m+2\ZZ$. One can also start from a negative integer $m\le-2$ and extend infinitely in the negative direction; the weights are $m-2\ZZ$. We denote these modules by $D_m^+$ and $D_m^-$, respectively.
		\item Limits of discrete series: there are irreducible Verma modules starting from $m=1$ and $m=-1$, as in the previous point. We denote these modules by $D_1^+$ and $D_1^-$, respectively.
		\qedhere
	\end{itemize}
\end{proof}
...
\begin{theorem}[Harish-Chandra]
	...
	\[Z(U\mf g)\cong\op{Sym}(\mf h)^{(W,\cdot)}.\]
	Here, the $(W,\cdot)$-action on $\mf h$ is $(-\rho)$-shifted, which means that $W$ fixes $(-\rho)$ (instead of the origin).
\end{theorem}
\begin{proof}[Sketch]
	...
	\end{example} 
	It is notably not obvious that this map even lands in $(U\mf h)^{(W,\cdot)}$, but once we have done so, we remark that we can check that our ring homomorphism is an isomorphism of vector spaces by comparing it with the isomorphism of vector spaces produced with Chevalley's theorem.

	Let's explain how one shows that $(U\mf g)_0/I$ maps to $\op{Sym}(\mf h)^W$. This requires a different construction of the Harish-Chandra isomorphism, using Verma modules. Fix some weight $\lambda\in\mf h^*$, and then there is a Verma module $M_\lambda$. It turns out that $Z(U\mf g)$ acts by a fixed scalar on $M_\lambda$, essentially by using properties of highest weights, and it turns out that this scalar is the value of $\lambda$ under the polynomial produces by the Harish-Chandra isomorphism. To show that this scalar does not depend on the $(W,\cdot)$-orbit of $\lambda$, one can reduce t to the $\mf{sl}_2$-theory with simple reflections.
\end{proof}
\begin{example}
	Let's explicate the last paragraph for $\mf{sl}_2$. It turns out that there is a short exact sequence
	\[0\to M_{-\lambda-2}\to M_\lambda\to L_\lambda\to0\]
	of representations. Thus, the Casimir $\Omega$ acts on $M_{-\lambda-2}$ and $M_\lambda$ by the same scalar!
\end{example}
