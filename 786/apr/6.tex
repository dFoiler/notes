...
\begin{definition}[automorphic form]
	Fix a function field $F$, and let $G$ be a split reductive group. Let $G(\AA_F)$ act on $C^\infty(G(F)\backslash G(\AA_F))$ by right translations, denoted $R$. Then $f\in C^\infty(G(F)\backslash G(\AA_F))$ is an \textit{automorphic form} if and only if
	\[\op{span}\{R(g)f:g\in G(\AA_F)\}\]
	is an admissible representation of $G(\AA_F)$. We let $\mc A(G,K)$ be the space of automorphic forms of level $K$.
\end{definition}
Unsurprisingly, we have a notion of cusp forms.
\begin{definition}[cuspidal]
	Fix a function field $F$, and let $G$ be a split reductive group. An automorphic form $f$ is \textit{cuspidal} if and only if
	\[\int_{N(F)\backslash N(\AA_F)}f(ng)\,dn=0\]
	for all $g\in G(\AA_F)$ and all proper parabolic subgroups $P\subseteq G$, whose unipotent radical is denoted $N$. We let $\mc A_{\mathrm{cusp}}(G,K)$ be the space of cusp forms of level $K$.
\end{definition}
...
\begin{exe} \label{exe:double-quotient-as-groupoid}
