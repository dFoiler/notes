...
We are now ready to sketch multiplicity one.
\begin{theorem}[Multiplicity one]
	Fix a cuspidal automorphic form $\pi$ for the group $\op{GL}_2$ over a function field $F$. Then $\pi$ is uniquely embedded in $\mc A_{\mathrm{cusp}}$.
\end{theorem}
\begin{proof}
	There are two steps: we will show that $\pi$ has a Whittaker model, and then we will show that the Whittaker model actually determines $\pi$. Set $G\coloneqq\op{GL}_2$ for brevity.

	Let's start by hunting for a Whittaker functional. We start with some $\varphi\in V_\pi$, which is lives in the space of functions $G(F)\backslash G(\AA_F)\to\CC$, and we are hunting for an invariant function for $N(\AA_F)$. Because $\psi_N$ already vanishes on $N(F)$, we are thus allowed to do the average
	\[\Lambda\colon\varphi\mapsto\int_{N(F)\backslash N(\AA_F)}\varphi(u)\psi(u)^{-1}\,du.\]
	Notably, the space $N(F)\backslash N(\AA_F)$ is an iterated extension of $F\backslash\AA_F$, so it is compact. One needs to check that $\Lambda$ is nonzero, which uses cuspidality. From this functional $\Lambda$, there is a map $V_\pi\to\mc A((N(\AA_F),\psi_N)\backslash G(\AA_F))$ given by sending $\varphi$ to the function $g\mapsto\Lambda(\pi(g)\varphi)$. Notably, it is not immediately obvious that this has moderate growth in the function field case, but it does.

	We do not have time to discuss the second step, which is related to the Fourier transform.
\end{proof}
