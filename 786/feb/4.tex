...
\begin{remark}
	...
	\[\OO_F^\times\to(F\otimes_\QQ\RR)^\times\to F^\times\backslash\AA_F^\times\mathbin{\bigg/}\prod_{v\nmid\infty}\OO_v^\times\to\op{Cl}F\to0.\]
\end{remark}
...
\[1\to\mu(F)\to\OO_F^\times\to(F\otimes_\QQ\RR)^{\times,1}\to F^\times\backslash\AA_F^{\times,1}\mathbin{\bigg/}\prod_{v\nmid\infty}\OO_v^\times\to\op{Cl}F\to0,\]
...
\begin{theorem} \label{thm:idele-class-group}
...
\end{remark}
\begin{remark}
	A similar argument shows that each local field $F_v$ has $F_v^*\cong F_v$, again given by a choice of character $\psi_v\in F_v^*$. Namely, there is a map $F_v\to F_v^*$ given by sending $x\in F_v$ to the character $y\mapsto\psi_p(xy)$.
\end{remark}
...
\end{remark}
\begin{remark} \label{rem:self-dual-haar}
