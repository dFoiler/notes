...
\begin{theorem}[Poisson summation] \label{thm:global-poisson-summation}
...
\end{remark}
\begin{remark}
	Having a self-duality means that the Poisson summation formula actually automatically implies $\op{vol}(F\backslash\AA_F;dx)=1$, provided that $dx$ is the self-dual Haar measure. Roughly speaking, $\prod_v\OO_v$ includes into this space (up to some controlled kernel) with finite but likely large cokernel. We thus expect that $\prod_v\OO_v$ to have volume at most $1$, so we expect each of the $\OO_v$s to have volume at most $1$. This is a useful sanity check for when we compute the self-dual Haar measure on the local fields $F_v$.
\end{remark}
