...
Let's make a few remarks about functoriality.\todo{What is the affine group?}
\begin{notation}
...
\end{example}
\begin{remark}
	One can check that the natural map $V\to(V^\lor)^\lor$ is an isomorphism if $V$ is admissible. Indeed, because $V=\bigcup_{K\subseteq G}V^K$, it is enough to check that the canonical map is an isomorphism after passing to $K$-invariants, where it is true because everything is finite-dimensional.
\end{remark}
...
\begin{definition}[Hecke algebra]
	Fix a totally disconnected group $G$. Then we define the \textit{Hecke algebra} $\mc H$ to be $C_c^\infty(G)$. Upon fixing a left Haar measure $dg$, we may define a convolution operation by
