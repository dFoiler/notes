...
\begin{definition}
	...
	\[L_v(\chi_v)\coloneqq\begin{cases}\frac1{1-\chi_v(\varpi_v)} & \text{if }\chi_v|_{\OO_v^\times}=1, \\0&\text{otherwise},\end{cases}\]
	and
...
\end{definition}
\begin{remark}
	Of course, there is redundancy in our notation: indeed, $Z(s+t,\chi,f)=Z(s,\chi\left|\cdot\right|^t,f)$.
\end{remark}
\begin{theorem}[Tate] \label{thm:tate}
	Fix a global field $F$ and some $f\in\mc S(\AA_F)$.
	\begin{listalph}
		\item The function $\chi\mapsto Z(\chi,f)$ admits a meromorphic continuation and a functional equation
		\[Z(\chi,f)=Z\left(\chi^{-1}\left|\cdot\right|,\widehat f\right).\]
		\item If $\chi$ is nontrivial on $\AA_F^{\times,1}$, then $s\mapsto Z(s,\chi,f)$ is holomorphic.
		\item The function $s\mapsto Z(s,1,f)$ is holomorphic everywhere except for simple poles at $s\in\{0,1\}$ with residue $-f(0)\op{vol}\big(F^\times\backslash\AA_F^{\times,1};d^\times x\big)$ at $s=0$.
	\end{listalph}
\end{theorem}
% \begin{remark}
...
\begin{theorem} \label{thm:fe-dedekind-zeta}
...
\begin{example}
	...
	Further suppose that $\chi|_{\op{Pic}^0X}\ne1$. Then we claim that $L(s,\chi)$ is a polynomial in $q^{-s}$. Indeed, in light of the above expansion, it is enough to show that all line bundles of large degree $d$ appear an equal number of times as $\OO_X(D)$ as $D$ varies over effective divisors of $d$. But this is true because $d>2g-2$ makes $X^{(d)}$ a $\PP^{d-g}$-bundle over $\op{Pic}^dX$: by an argument with linear systems (and the Riemann--Roch theorem), the fibers are all copies of $\PP^{d-g}=\PP\mathrm H^0(X;\OO_X(D))$.
\end{example}
