...
\end{notation}
\begin{remark}
	The continuity of $\chi$ forces $\chi_v|_{\OO_v^\times}=1$ for all but finitely many $v$: indeed, $\prod_{v\nmid\infty}\OO_v^\times$ is a profinite group, so a ``no small subgroups'' argument shows that $\ker\chi$ must contain an open subgroup of $\prod_{v\nmid\infty}\OO_v^\times$. Conversely, given a family $\{\chi_v\}_{v\in V(F)}$ of continuous characters for which $\chi_v|_{\OO_v^\times}=1$ for all but finitely many $v$, the product character $\chi\coloneqq\prod_v\chi_v$ on $\AA_F^\times$ makes sense, and it is continuous because the kernel contains an open subgroup of $\prod_{v\nmid\infty}\OO_v^\times$ (and hence factors through a finite quotient of that group).
\end{remark}
