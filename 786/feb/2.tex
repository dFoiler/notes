...
\begin{remark}
	...
	\[F\backslash\AA_F/\prod_{v\in V(K)}\OO_v\stackrel?\cong\mathrm H^1(X;\OO_X).\]
	In particular, the right-hand side will frequently have more than one element: it has dimension over $\FF_q$ equal to the genus of $X$, so the cohomology group has one element if and only if $X$ is $\PP^1_{\FF_q}$! To see this isomorphism, note that $\AA_F/\prod_v\OO_v$ is simply $\bigoplus_vF_v/\OO_v$. Then there is a short exact sequence
	\[0\to\OO_X\to\mc K\to\bigoplus_{v\in V(F)}i_{v*}(F_v/\OO_v)\to0,\]
	where $\mc K$ is the constant sheaf of rational functions. Taking the long exact sequence in cohomology discovers the desired isomorphism.\todo{}
\end{remark}
