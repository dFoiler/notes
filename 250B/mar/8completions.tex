...
\end{proof}
\begin{remark}[Nir]
	One might complain that this proof seems needless hard, but there is in fact some obstacle to overcome. For example, when one wants to show that $k[[x]]$ is Noetherian (which is the completion of $k[x]$ at $(x)$), one usually has to use Weierstrass preparation, which is also a difficult technique.
\end{remark}
