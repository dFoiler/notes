...
\end{remark}
\begin{example} \label{ex:projresex}
...
\end{lemma}
\begin{proof}
	We will be brief. Fix (augmented) projective resolutions for $M$ and $N$ named $\left(P,\del^P\right)$ and $\left(Q,\del^Q\right)$ with augmentation maps $P\stackrel{\varepsilon_M}\to M$ and $Q\stackrel{\varepsilon_N}\to N$, respectively. The point is to draw the following big diagram.
...
\begin{lemma} \label{lem:flattorbasechange}
...
\begin{exe} \label{exe:torquotientcomputation}
