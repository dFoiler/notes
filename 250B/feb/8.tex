...
\begin{proof}
	...
	\begin{lemma}
		Fix $R$ a ring and $\{\mf p_k\}_{k=1}^n$ and $\mf p$ some prime ideals. Then if
		\[\bigcap_{k=1}^n\mf p_k\subseteq\mf p,\]
		then $\mf p_k\subseteq\mf p$ for some $\mf p_k$.
	\end{lemma}
	\begin{proof}
		We prove this by contraposition. Suppose that $\mf p_k\not\subseteq\mf p$ for each $\mf p_k$ so that we can find $x_k\in\mf p\setminus\mf p_k$. \todo{}
	\end{proof}
	\begin{remark}[Nir]
