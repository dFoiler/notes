...
Here we go.
\begin{warn}
	For today's lecture, an $S$-algebra $R$ should be thought of as providing an embedding $R\into S$. We will even think $R\subseteq S$.
\end{warn}
...
\begin{definition}[Finite]
	Fix $S$ an $R$-algebra. Then $S$ is \textit{finite} over $R$ if and only if $S$ is finitely geneated over $R$ (as an $R$-module).
\end{definition}
