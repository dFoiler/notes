...
\begin{proof}
	...
	\begin{lemma}[Prime avoidance] \label{lem:primeavoidance}
		Fix a ring $R$ and ideals $I_1,\ldots,I_n$ of $R$, at most two of which are not prime. If an ideal $H$ has $J\not\subseteq\mf I_k$ for each $I_k$, then $J$ contains an element not in any of the $I_k$.
	\end{lemma}
	\begin{proof}
		We induct on $n$. If $n=0$, then $0\in J$ will do. If $n=1$, then we just say that $J\setminus I_1$ is nonempty, finishing.

		Now take $n\ge2$. By the induction, we are promised elements
		\[x_\ell\in J\mathbin{\bigg\backslash}\bigcup_{k\ne\ell}I_k.\]
		For psychological reasons, suppose for the sake of contradiction that $J$ is in fact contained in the union of the $I_k$. Then we must have $x_\ell\in I_\ell$ for each $\ell$. We now have two cases to finish our inductive step.
		\begin{itemize}
			\item If $n=2$, then
			\[x_1+x_2\equiv x_2\not\equiv0\pmod{I_1}\qquad\text{and}\qquad x_1+x_2\equiv x_1\not\equiv0\pmod{I_2},\]
			so $x_1+x_2\in J\setminus(I_1+I_2)$, finishing the proof. (Technically, this is our contradiction.)
			\item If $n>2$, then at least one of the $I_k$ is prime, so we may rearrange the $I_k$ so that $I_1$ is prime. But then $y\coloneqq x_1+x_2x_3\cdot\ldots\cdot x_n$ has
			\[y\equiv x_2x_3\cdot\ldots\cdot x_n\not\equiv0\pmod{I_1}\qquad\text{and}\qquad y\equiv x_1\pmod{I_k}\text{ for }k>1.\]
			In particular, $x_2x_3\cdot\ldots\cdot x_n\notin I_1$ because each of the $x_k$ is not in $I_1$ and $I_1$ is prime. Thus, $y$ will now provide our contradiction.
		\end{itemize}
