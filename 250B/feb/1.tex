...
\begin{proof}[Proof of the forwards direction of \autoref{thm:artingrabbag}]
	...
	We now proceed with the proof of \autoref{thm:artingrabbag}. The payoff to \autoref{lem:artinianfinmaxes} is that the Chinese remainder theorem gives us right-exactness of the short exact sequene
	\[0\to J\to R\to\prod_\mf mR/\mf m\to 0.\]
	In particular,
	\[R/J\cong\prod_\mf mR/\mf m\]
	is a product of finitely many simple modules $R/\mf m$, so $R/J$ will be of finite length. (In particular, $R/J$ has only finitely many ideals because each $R/\mf m$ has only two ideals.) We would like to build use this result on $R/J$ to tell us that $R$ is of finite length, but we will need a smallness condition on $J$ to make this work.
	...
	By \autoref{cor:finlengthquotient}, it suffices to check that $J^i/J^{i+1}$ is of finite length, for each $i$. But observe that we have the short exact sequences
