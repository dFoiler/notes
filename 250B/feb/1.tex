...
\begin{proof}
	...
	\end{tikzcd}\]
	To be explicit, the induced arrow is creaeted by pulling back $(R/I)\otimes_RM$ to $R\otimes_RM$, then pushing forward through to $M$ and then $M/IM$. Explicitly, we take
	\[[r]_I\otimes m\mapsfrom r\otimes m\mapsto rm\mapsto[rm]_{IM}.\]
	Well-definedness is by the commutativity and exactness of the diagram: if $r\equiv s\pmod I$, then $r\otimes m$ and $s\otimes m$, but $(r-s)\otimes m$ is in the kernel of $R\otimes_RM\to(R/I)\otimes_RM$, so $(r-s)m$ is in the kernel of $M\to M/IM$, so $[rm]_{IM}=[sm]_{IM}$.
...
\end{prop}
\begin{proof}
	We use the embedding $A\into B$ to view $A$ as a $R$-submodule of $B$, and we use the projection $B\onto C$ to view $C\cong B/A$ as a quotient. We now take the directions independently.
	\begin{itemize}
...
\begin{proof}
	...
	However, if the length of $A$ were equal to the length of $B$, then in the process of removing redundences from \hyperref[eq:compseriesa]{($*$)} must not do anything at all, for any removed redundancy would imply that the length of $A$ is strictly less than the length of $B$.
...
\begin{corollary}
	Fix $M$ an $R$-module. Them $M$ is of finite length if and only if $M$ is both Artinian and Noetherian.
\end{corollary}
\begin{proof}
	We will be brief; the backwards direction is \autoref{lem:artnoethimpliesfinlength}. For the forwards direction, suppose $M$ has a composition series with $n$ composition factors. The point is that composition series are maximal among all chains of distinct submodules, so any chain of submodules can have at most $n+1$ distinct submodueles. In particular, any ascending or descending chain must stabilize.
\end{proof}
...
\begin{lemma}
	...
	\[M:=M_0\supsetneq M_1\supsetneq\cdots\supsetneq M_n:=\{0\}.\]
	If the composition factors are $\{R/\mf p_1,\ldots,R/\mf p_n\}$, then $\op{Supp}M=\{\mf p_1,\ldots,\mf p_n\}$.
\end{lemma}
\begin{proof}
