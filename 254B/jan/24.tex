...
\begin{definition}[CM type]
	...
	and in fact $E$ acts on $\op{Lie}A(\CC)$. Decomposing $\op{Lie}A(\CC)$ as an $E$-representation as $\bigoplus_{\varphi\in\Phi}\CC_\varphi$ where $\Phi\subseteq\op{Hom}(E,\CC)$.\todo{Why does this decompose?} Then $\Phi$ is the CM type.
\end{definition}
\begin{remark}
...
\end{proposition}
\begin{proof}
	The point is to restrict \Cref{cor:factor-ab-var} to simple abelian varieties. We won't give a full proof today, but we will gesture towards an idea: essentially, if $(E,\Phi)$ is an extension of $(E_0,\Phi_0)$, then
