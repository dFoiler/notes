...
\end{proof}
We now see that the equivalence of (a) and (d) implies that the map sending $A$ to the conjugacy class of $q$-Weil numbers given by $\op{Frob}_A$ will be injective. This is the injectivity required in \Cref{thm:ht}. Technically, there is something to be checked because we need to be able to read the degree of $P_A$ and $P_B$ off of just knowing one root of it, which will follow from the relevant simplicity.
