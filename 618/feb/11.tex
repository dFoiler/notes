...
\end{proof}
\begin{remark}
	The Galois descent described above basically comes from the following example: suppose $V=\OO$, and given a $1$-cocycle $c\colon W_K\to\OO^\times$ given by sending Frobenius to some unit $u\in\OO^\times$. Then we get a new action $\star$ of Galois on $V_{\widehat\OO}$ given by $\sigma\star v\coloneqq u\cdot\sigma v$; we claim that the $\star$-fixed points $V_{\widehat\OO}^{\sigma\star}$ is still $\OO$. This boils down to something analogous to Hilbert's theorem 90.
\end{remark}
\begin{example}
	Let's try to compute $\mc G_{-p}$ for $K=\QQ_p$. Once one writes out the Galois cocycle, one finds that $\mc G_{-p}$ is $\widehat{\mathrm U}_1$, where $\mathrm U_1$ is the kernel of the norm map $\op N\colon\mathbb G_{m,\QQ_p^2}\to\mathbb G_{m,\QQ_p}$. To be explicit, one should write $\QQ_{p^2}=\QQ_p(\sqrt D)$ for some $D$, and then we see that $\mathrm U_1(\ZZ_p)$ consists of pairs $(X,Y)$ (thought of as $X+Y\sqrt D$) such that $X^2-DY^2=1$.
\end{example}
...
This will more or less follow formally from having $\mc G_{\varpi,\widehat\OO}\cong\mc G_{\varpi',\widehat\OO}$. Approximately speaking, we can think about these both as groups on $\widehat\OO[[X]]$, and there is some Galois cocycle relating the two.

% For brevity, set $\widehat K_n\coloneqq\widehat KK_n$. Note that $\widehat K_n^{\op{Art}_K(\varpi)}$ includes $K_n$, and it must actually be equal to $K_n$ because $\op{Art}_K(\varpi)$ acts nontrivially (in particular, as Frobenius) on $\widehat K$.
We would like to show that $\eta_\varpi(\op{Art}_K\varpi')=u$, where $u$ is chosen with $\varpi'=\varpi u$, and we already know that $\eta_{\varpi'}(\op{Art}_K\varpi')=1$. Thus, we will be done if we can show a statement of the form
\[\eta_{\varpi'}(\tau)\stackrel?=\eta_\varpi(\tau)u^{\op{val}(\tau)},\]
where $\op{val}$ is the valuation of $\mf p$. Now using the $1$-cocycle from earlier, we know that $\tau\star u=[u]^{\mathrm{val}(\tau)}\cdot\tau x$, where the left action is the one given by $\varpi'$ and the right one is given by $\varpi$. This translates into the desired equality, so we are done.
