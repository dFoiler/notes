...
\begin{definition}[Tate module]
	...
	for any $n\ge0$. We also write $\mc G[\mf p^\infty]\coloneqq\bigcup_{n\ge0}\mc G[\mf p^n]$ and the Tate module $T_{\mf p}\mc G=\limit\mc G[\mf p^\bullet]$.
\end{definition}
\begin{remark}
...
\begin{theorem} \label{thm:lt}
	Fix a $p$-adic field $K$ with ring of integers $\OO$ and maximal ideal $\mf p\subseteq\OO$, and let $\varpi\in\mf p$ be a uniformizer. Then there is a unique formal $\OO$-module $\mc G_\varpi$ (up to isomorphism) such that
	\[K_\pi\coloneqq K(\mc G[\mf p^\infty])\]
...
\end{theorem}
\begin{remark}
	Let's give some indication of what is going on under the hood. We will find that $T_\mf p G_\varpi$ is a free rank-$1$ module over $\OO$. Then the Galois action of $\op{Gal}(K^{\mathrm{ab}}/K)$ on $T_{\mf p}G_{\varpi}$ will induce a character $\eta_\varpi\colon\op{Gal}(\ov K/K)\to\OO^\times$ defined by
	\[\eta_\varpi(\sigma)\cdot v=\sigma(v)\]
	for any $v\in T_{\mf p}G_\varpi$. We will show that $\eta_\varpi$ is characterized by having $\op{Art}_K^{-1}(a)=\varpi^\bullet\eta_\varpi(a)$ for some power $\varpi^\bullet$.
\end{remark}
\begin{remark}
...
\begin{remark}
	Note $\mc G_E$ has an $\mathcal O_K$-action coming from $E$, and $T_\mf p\mc G_E$ also has an action by $\OO_v$, but it is not clear that these actions will agree.
\end{remark}
...
\begin{remark}
	The proof above raises a bizarre question: what is the element $\mu\in\OO_K$ which induces the Frobenius action by $\mathrm{Frob}_{\mf P}$ on $E$? However, this question does not make sense: the hunt for an element $\mu$ depends on a choice of model of $E$ over $H$.
	
	Letting $E$ be defined over the Hilbert class field $H$ of $\OO_K$, we note that its model is not unique and indeed depends on the choice of cocycle in the continuous cohomology group
	\[\mathrm H^1(\op{Gal}(\ov\QQ/H),\op{Aut}_\OO(E)).\]
	Because $\op{Aut}_\OO(E)=\OO^\times$, we see that the Galois action is trivial, so this is $\op{Hom}_{\mathrm{cont}}(\op{Gal}(\ov\QQ/H),\OO^\times)$. Then the reduction$\pmod{\mf P}$ of $E$ is going to be defined up to an element of
...
\begin{definition}[Lubin--Tate formal group]
	Fix a $p$-adic field $(K,\OO,\mf p)$. For a given uniformizer $\varpi\in\mf p$ and $f\in\mc F_\varpi$, we define the \textit{Lubin--Tate formal group} $\mc G_f$ to be the unique commutative $1$-dimensional formal group $\mc G$ over $\OO$ such that $f$ is an endomorphism.
\end{definition}
