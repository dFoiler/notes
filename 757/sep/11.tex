...
\end{lemma}
\begin{proof}
	% Let's begin with uniqueness. By linearity, there is a unique bilinear map
	% \[\boxtimes\colon\op{Meas}^0_c(X)\times\op{Meas}^0_c(Y)\to\op{Meas}_c(X\times Y)\]
	% such that $\delta_x\boxtimes\delta_y\coloneqq\delta_{(x,y)}$. Then \Cref{prop:dirac-dense} implies that $\op{Meas}_c^0(X)\subseteq\op{Meas}_c(X)$ and $\op{Meas}_c^0(Y)\subseteq\op{Meas}_c(Y)$ are sequentially dense subsets.
	%
	% Now, for any $\mu\in\op{Meas}_c(X)$ and $\nu\in\op{Meas}_c(Y)$, we would like to show that $\mu\boxtimes\nu$ is unique given that $\boxtimes$ is further continuous in both variables. Well, we may find sequences $\{\mu_i\}\in\op{Meas}_c^0(X)$ and $\{\nu_j\}\in\op{Meas}_c(Y)$ such that $\{\mu_i\}\to\mu$ and $\{\nu_j\}\to\nu$. Now, continuity in each variable implies that $\{\mu_i\boxtimes\nu\}_i\to\mu\boxtimes\nu$ while $\{\mu_i\boxtimes\nu_j\}_j\to\mu_i\boxtimes\nu$ for each $i$. Thus,
	% \[\mu\boxtimes\nu=\lim_{j\to\infty}\left(\lim_{i\to\infty}\mu_i\boxtimes\nu_j\right),\]
	% which is unique if it exists. We conclude that $\mu\boxtimes\nu$ is uniquely defined!
	%
	...
	By \Cref{prop:extend-cont}, it is enough to check that $\boxtimes\colon\op{Meas}^0_c(X)\times\op{Meas}^0_c(Y)\to\op{Meas}_c(X\times Y)$ is sequentially continuous. Thus, fix Cauchy sequences $\{\mu_i\}\to\mu$ and $\{\nu_j\}\to\nu$, and we want to check that $\{\mu_i\boxtimes\nu_j\}\to\mu\boxtimes\nu$ as $i,j\to\infty$. Quickly, we remark that we can reduce to the case where $X$ and $Y$ are compact: let $a\colon K\subseteq X$ be a compact subset containing the supports of $\mu$ and the $\mu_\bullet$s, and define $b\colon L\subseteq Y$ similarly. (These exist by \Cref{lem:cauchy-seq-supp-in-compact}.) Then \Cref{lem:pull-push} allows us to restrict all measures on $X$ to $K$ and restrict all measures on $Y$ to $L$ (and all measures on $X\times Y$ to $K\times L$), thereby reducing to the case that $X$ and $Y$ are compact.
	...
	% \begin{enumerate}
	% 	\item For any fixed $\mu\in\op{Meas}_c^0(X)$, we claim that $(\mu\boxtimes-)\colon\op{Meas}_c^0(Y)\to\op{Meas}_c(X\times Y)$ is continuous; by \Cref{prop:extend-cont}, we are then able to extend $(\mu\boxtimes-)$ to a continuous map with domain $\op{Meas}_c(Y)$. Indeed, choose a convergent net $\{\nu_i\}\to\nu$ in $\op{Meas}_c^0(Y)$, and we want to check that $\{\mu\boxtimes\nu_i\}\to(\mu\boxtimes\nu)$. By the ambient bilinearity of $\boxtimes$, we may assume that $\mu=\delta_x$ for some $x\in X$.
	%
	% 	Now, for any $f\in C(X\times Y)$, we would like to show that $\{(\mu\boxtimes\nu_i)(f)\}\to(\mu\boxtimes\nu)(f)$. By a quick expansion of $\nu_i$ as $\sum_jc_{ij}\delta_{y_{ij}}$ for some constants $\{c_{ij}\}_j$ and points $\{y_{ij}\}$, we see that
	% 	\[(\mu\boxtimes\nu_i)(f)=\nu_i(f_x),\]
	% 	where $f_x\colon Y\to\CC$ is defined as $f_x(y)\coloneqq f(x,y)$. Note $f_x\in C(Y)$, so we know that $\{\nu_i(f_x)\}\to\nu(f)$ because $\{\nu_i\}\to\nu$.
	%
	% 	\item For any fixed $\nu\in\op{Meas}_c(Y)$, we claim that $(-\boxtimes\nu)\colon\op{Meas}_c^0(X)\to\op{Meas}_c(X\times Y)$ is continuous; by \Cref{prop:extend-cont}, we are then able to extend $(-\boxtimes\nu)$ to a continuous map with domain $\op{Meas}_c(X)$.
	%
	% 	Once again, for any net $\{\mu_i\}\to\mu$ in $\op{Meas}_c^0(X)$, we would like to check that $\{\mu_i\boxtimes\nu\}\to\mu\boxtimes\nu$. In other words, for any fixed function $f\in C(X\times Y)$, we would like to show that $\{(\mu_i\boxtimes\nu)(f)\}\to(\mu\boxtimes\nu)(f)$. Now, by definition, $(-\boxtimes\nu)(f)=(-\boxtimes\nu_j)(f)$ for some chosen Cauchy sequence $\{\nu_j\}\to\nu$ where $\nu_j\in\op{Meas}_c^0(X)$.
	%
	% 	To complete the argument, fix an error tolerance $\varepsilon>0$. Thus, for large $j$, we see that $(\mu\boxtimes\nu)(f)$ is with $\varepsilon$ of $(\mu\boxtimes\nu_j)(f)$. But now $(-\boxtimes\nu_j)$ is continuous on $\op{Meas}_c^0(X)$ by applying the previous step symmetrically, so $(\mu\boxtimes\nu_j)(f)$ is within $\varepsilon$ of $(\mu_i\boxtimes\nu_j)(f)$ for large $i$. Lastly, $(\mu_i\boxtimes-)$ is continuous, so $(\mu_i\boxtimes\nu_j)(f)$ is within $\varepsilon$ of $(\mu_i\boxtimes\nu)(f)$ for large $j$. Thus, choosing $i$ and $j$ large enough, we see that
	% 	\[\left|(\mu_i\boxtimes\nu)(f)-(\mu\boxtimes\nu)(f)\right|\le3\varepsilon,\]
	% 	so the result follows by sending $\varepsilon\to0^+$.
	%
	% 	\item It remains to check that the map $(\mu\boxtimes-)\colon\op{Meas}_c(Y)\to\op{Meas}_c(X\times Y)$ is continuous for any $\mu$. This is simply a more involved version of the argument in the second step, so we will omit it.
	% \end{enumerate}
\end{proof}
\begin{notation}
