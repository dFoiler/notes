...
We are now ready to state our theorem, whose proof is in \Cref{sec:pw}.
...
\end{definition}
It turns out that these matrix coefficient maps are smooth, which is shown in \Cref{prop:smooth-dense}.
\begin{example}
...
\end{proposition}
\begin{proof}
	We argue as in \Cref{thm:char-ortho}. To begin, we see
	...
	by considering the coefficients of the tensor product operator. However, $\rho_V\otimes\rho_{\ov W}=\rho_{V\otimes W^*}$ by elaborating on \Cref{rem:char-sum-tensor,rem:char-dual}, so we see that we are trying to compute a matrix coefficient of the operator
	\[R\coloneqq\int_G\rho_{V\otimes W^*}(g)\,dg.\]
...
\begin{proof}
	...
	As usual, we note that $\rho_V\left(h^{-1}\right)_{ji}=\overline{\rho_V(h)}_{ij}$, which is seen by expanding a basis. Then the integral vanishes by \Cref{prop:matrix-coef-ortho} unless $V\cong W$, which explains the coefficient $\langle\chi_V,\chi_W\rangle$ in the right-hand side of the statement of the corollary.
	
	Otherwise, in the event that $V\cong W$, we may as well identify them and use the same orthonormal bases. Then \Cref{prop:matrix-coef-ortho} tells us that the integral still vanishes unless $(j,i)=(k,k)$, in which case the value is $1/\dim V$. Thus, we are left with the sum
...
\begin{example} \label{ex:char-avg-proj}
	...
	is $\id_V$. Indeed, a direct calculation with the invariance of the Haar measure shows that this operator is $G$-invariant, so it must be a scalar. To compute that this scalar is the identity, we can check that the trace of this operator equals $\dim V$. Indeed, \Cref{cor:character-convolution} explains the trace is equal to $(\chi_V*\chi_V)(1)=\dim V$.
\end{example}
