...
Thus, we now turn to the principal series $V_s^\pm$. In fact, we will more generally. Let $G$ be a split real reductive group with torus $T$, and let $B\subseteq G$ be a Borel, and let $U\subseteq B$ be the unipotent radical. Let $\theta$ be some character on $T$, and we consider the induction $V_\theta$ as functions on $G/U$ satisfying the transition $f(xt)=\theta(t)^{-1}f(x)$.
% As usual, identify $V_s^\pm$ with functions on $\RR^2\setminus\{0\}$ with some homogeneity properties, and we want to compute the action of some $f\in C_c^\infty(G)$ on $V_s^\pm$. Explicitly, $V_s$ consists of functions $f_0\colon\left(\RR^2\setminus\{0\}\right)\to\CC$ for which $f_0(tz)=\pm\left|t\right|^{-is-1}f_0(z)$.
We will compute our character by chasing around the following diagram.
...
We are now able to compute our character $\chi$.
% Now, by ``integrating'' over the fiber of $g$ in $\widetilde G$, one finds that $\chi_V(g)$ vanishes if $g$ is not diagonalizable over $\RR$; otherwise,
% \[\chi_V(g)\stackrel?=\frac1{\prod_{\alpha\in\Phi^+}\left|1-\alpha(g)\right|}\sum_w\nu(wg),\]
% where $\nu$ is $-is-1$ from earlier.
Let's explain this. Suppose that $f$ is supported on the regular semisimple elements, and we want to compute $\tr(f\,dg;V_\theta)$, where $V_\theta$ consists of functions on $G/U$ for which $f(xt)=\theta(t)f(x)$. Then
\begin{align*}
	\tr(f\,dg;V_\theta) &= \int_{\widetilde G}p^*(\theta)\pi^*(f)\,d\widetilde g \\
	&= \int_G\widetilde f\,dg,
\end{align*}
where
\[\widetilde f(g)=f(g)\sum_{\widetilde g_w\in\pi^{-1}(g)}\theta(p(\widetilde g_w))\frac{d\widetilde g_w}{dg}.\]
Thus,
\[\chi(g)=\sum_{\widetilde g_w\in\pi^{-1}(g)}\theta(p(\widetilde g_w))\frac{d\widetilde g_w}{dg}.\]
To compute $d\widetilde g_w/dg$, we note that $dg/d\widetilde g_w$ is simply $\left|\det d\pi_g\right|$. To compute this determinant, write $\widetilde g_w=(g,B_w)$, and we note that the bundle sequence $B\to \widetilde G\to G/B$ induces a morphism
...
\[\begin{tikzcd}[cramped]
	0 & {\mf b_w} & {T_{(g,B_w)}\widetilde G} & {\mf g/\mf b_w} & 0 \\
	0 & {\mf b_w} & {T_gG} & {\mf g/\mf b_w} & 0
...
\end{tikzcd}\]
of short exact sequences, and the right-hand map turns out to be given by $\mathrm{Ad}_g-1$. (Observe that this short exact sequence explains why $d\widetilde g$ is well-defined: one can trivialize $\land^{\mathrm{top}}T_{\widetilde g}\widetilde G$ because it is $\land^{\mathrm{top}}(\mf g/\mf b_w)\otimes\land^{\mathrm{top}}(\mf b_w)=\land^{\mathrm{top}}\mf g$.) Notably, $\left|d\pi\right|$ can be computed on the trivial line bundle $\Omega_{\widetilde G}^{\mathrm{top}}$ as
\[\det\left(\mathrm{Ad}_g-1;\mf g/\mf b_w\right)=\prod_{\alpha\in\Phi^+_w}(1-e^\alpha(g)).\]
This determinant may have a different answer for different $w\in W$, but this turns out to be okay because one can absorb the difference into $\theta$. Notably, applying a simple reflection $s_\alpha$ replaces $1-e^\alpha$ with $1-e^{-\alpha}$, which is $-e^{-2\alpha}(1-e^\alpha)$. But $e^{-2\alpha}=e^{s_\alpha(2\rho)}/e^{2\rho}$, so we see that we can write
\[\chi=\Bigg(\sum_{w\in W}e^{2(\nu)}\Bigg)\prod_{\alpha\in\Phi^+}\left(e^{\alpha/2}-e^{-\alpha/2}\right)^{-1},\]
where $\nu$ is chosen so that $\theta=\nu\cdot e^{-\rho}$.
% The same sort of determinant argument works over $\RR$, but we now have to be careful about which regular semisimple $g\in G$ are actually in the image of $\pi$: one finds that $g$ is in the image of $\pi$ if and only if $g$ is diagonalizable over $\RR$ and with $\left|W\right|$ preimages.
\begin{example}
	For $\op{SL}(2,\RR)$ and $g=\op{diag}\left(a,a^{-1}\right)$, we get
	\[\pm\frac{\left|a\right|^\nu+\left|a\right|^{-\nu}}{\left|a-a^{-1}\right|}.\]
\end{example}
\begin{example}
	Suppose that $V_\nu$ consists of global sections of some line bundle $\mc L_\nu$ (considered as half-densities), where $\mc L_\nu$ is the usual induction from $G/U$. If $\nu$ is unitary, then one finds that $\left|\nu(g)\right|=1$ for all $g\in T$, so $V_\nu$ is unitary, where the inner product is given by
	\[\langle f_1,f_2\rangle=\int_{G/B}f_1(x)\overline{f_2(x)}\,dx.\]
\end{example}
\begin{remark}[parabolic induction]
	Some of this story works more generally. Let $P\subseteq G$ be a parabolic subgroup, and let $P=LU$ be the Levi decomposition (where $L$ is the Levi, and $U$ is the unipotent radical). Given an admissible representation $\rho$ of $L$, we may write $i_P^G(\rho)$ to be the global sections of the corresponding equivariant sheaf $\mc F_\rho$ (considered as half-densities), which concretely are the usual functions $G/U\to\rho$ satisfying some $L$-invariant transition:
	\[f(x\ell)=\rho^{-1}(\ell)f(x)e^{-\rho_P}(\ell).\]
	Now, if $\rho$ is unitary, one finds $i_P^G(\rho)$ is still unitary.
\end{remark}

\subsection{The Cartan Decomposition}
As usual, $G$ is a real reductive group.
\begin{proposition}[Cartan decomposition]
	Let $G$ be a real group with maximal compact $K$, and let $A$ be a maximal split torus. Then
	\[G=KA^+K,\]
	where $A^+=\{\exp(-x):x\in\mf a^+\}$, where $\mf a^+$ is the positive part of $\mf a$ (for the Cartan involution).
\end{proposition}
\begin{remark}
	Here, the $^+$ in $A^+$ is being used because $\mf a^+$ consists of those $a\in\mf a$ for which $\langle\alpha,a\rangle>0$ for each root $\alpha$ of in $\op{Lie}N$.
\end{remark}
\begin{proof}[Sketch]
